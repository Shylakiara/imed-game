...
function draw() {
	rect(0, 0, canvas.width, canvas.height, 'orange');
	rect(0, 150, canvas.width, 100, 'red');
	rect(squareX, squareY, squareS * 2, squareS, 'white');
	rect(playerX, playerY, playerS, playerS, 'blue');
	text('Score: ' + Math.floor(score), '30px Exo', 10, 30, 'blue');
	if (gameover) {
		rect(0, 0, canvas.width, canvas.height, 'pink');
		text('GAME OVER', '75px Exo', 200, 300, 'red');
		rect(250, 350, 100, 50, 'blue');
	}
}
